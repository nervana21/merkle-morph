...
use crate::channel::{compute_channel_commitment, hash, ChannelState};
use crate::errors::{Result, WalletError};
use crate::types::{Bytes32, ChannelId, WalletId};
...
impl WalletState {
    ...
    pub fn new(wallet_id: WalletId) -> Self {
        Self { wallet_id, channels: BTreeMap::new(), commitment: [0u8; 32], nonce: 0 }
    }
    ...
    pub fn from_channels(wallet_id: WalletId, channels: Channels) -> Self {
        ...
        Self { wallet_id, channels, commitment, nonce: 0 }
    }
    ...
    pub fn insert_channel(
        ...
        Ok(WalletState {
            ...
            nonce: prior_state.nonce.checked_add(1).ok_or(WalletError::WalletNonceOverflow)?,
        })
    }
    ...
    pub fn transfer_in_channel(&self, channel_id: ChannelId, amount: u64) -> Result<ChannelState> {
        let channel =
            self.channels.get(&channel_id).ok_or(WalletError::ChannelNotFound(channel_id))?;
        Ok(channel.apply_transfer(channel_id, amount).map_err(WalletError::from)?)
    }
}
...
mod tests {
    ...
    fn test_insert_channel() {
        ...
        assert!(matches!(result, Err(Error::Wallet(WalletError::WalletNonceOverflow))));
    }
    ...
    fn test_transfer_in_channel() {
        ...
        assert!(matches!(
            result,
            Err(Error::Wallet(WalletError::Channel(ChannelError::InsufficientBalance)))
        ));
    }
}

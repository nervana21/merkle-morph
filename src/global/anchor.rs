...
///
/// Implementations of `BitcoinAnchoring` can use ethos's `ProtocolBackend` trait
/// to support multiple Bitcoin backends without code duplication. For example:
///
/// ```rust,no_run
...
mod tests {
    use super::*;
    use crate::types::OP_RETURN_OFFSET_VERSION;

    #[test]
    fn test_encode_decode_roundtrip() {
        let global_root = [0x42u8; 32];
        let nonce = 12345u32;
        ...
        assert_eq!(
            encoded.len(),
            OP_RETURN_TOTAL_LEN,
            "Encoded data should be exactly {} bytes",
            OP_RETURN_TOTAL_LEN
        );

        let (decoded_root, decoded_nonce) =
            decode_op_return_data(&encoded).expect("Should decode successfully");
        assert_eq!(decoded_root, global_root);
        assert_eq!(decoded_nonce, nonce);
    }

    #[test]
    fn test_encode_decode_max_u32_nonce() {
        let global_root = [0x99u8; 32];
        let nonce = u32::MAX;
    ...
    #[test]
    fn test_encode_decode_zero_nonce() {
        let global_root = [0x00u8; 32];
        let nonce = 0u32;
    ...
    fn test_decode_invalid_magic() {
        ...
        invalid_data[0..4].copy_from_slice(b"XXXX");
        let result = decode_op_return_data(&invalid_data);
        assert!(result.is_err());
        assert!(result
            .expect_err("Expected error for invalid magic bytes")
    ...
    fn test_decode_wrong_length() {
        ...
        let result = decode_op_return_data(&short_data);
        assert!(result.is_err());
        let err_msg = result.expect_err("Expected error for wrong length data");
        assert!(
            err_msg.contains("must be exactly"),
            "Error should mention exact length requirement"
        );
        assert!(err_msg.contains("40"), "Error should mention actual length");

        // Test too long
        let long_data = vec![0u8; 42];
        let result = decode_op_return_data(&long_data);
        assert!(result.is_err());
        let err_msg = result.expect_err("Expected error for wrong length data");
        assert!(
            err_msg.contains("must be exactly"),
        ...
        assert!(err_msg.contains("42"), "Error should mention actual length");
    }

    #[test]
    fn test_encode_format() {
        let global_root = [0xAAu8; 32];
        ...
        // Check version
        assert_eq!(encoded[OP_RETURN_OFFSET_VERSION], 0u8);

        // Check global root (primary data comes before nonce)
        assert_eq!(&encoded[OP_RETURN_OFFSET_GLOBAL_ROOT..OP_RETURN_OFFSET_NONCE], &global_root);

        // Check nonce (little-endian u32)
        let nonce_bytes: [u8; OP_RETURN_NONCE_LEN] = encoded
            [OP_RETURN_OFFSET_NONCE..OP_RETURN_OFFSET_NONCE + OP_RETURN_NONCE_LEN]
            .try_into()
            .expect("Should be able to convert 4-byte slice to array");
        let decoded_nonce_u32 = u32::from_le_bytes(nonce_bytes);
        assert_eq!(decoded_nonce_u32, 0x12345678u32);
    }

    #[test]
    fn test_decode_unsupported_version() {
        let global_root = [0xABu8; 32];
        let nonce = 42u32;

        let mut encoded = encode_op_return_data(global_root, nonce);
        encoded[OP_RETURN_OFFSET_VERSION] = 1; // bump version to unsupported value

        let err = decode_op_return_data(&encoded).expect_err("Should reject unsupported version");
        assert!(

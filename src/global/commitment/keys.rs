...
pub fn encode_node_key(wallet_id: &WalletId, depth: u8) -> Vec<u8> {
    ...
    let prefix_bytes = (depth as usize).div_ceil(8).clamp(1, 32);
...
pub fn decode_node_key(key: &[u8]) -> Option<(u8, [u8; 32])> {
    ...
    let prefix_len = (depth as usize).div_ceil(8).clamp(1, 32).min(key.len() - 1);
...
pub fn compute_prefix_bytes(wallet_id: &WalletId, depth: u8) -> Vec<u8> {
    let prefix_bytes = (depth as usize).div_ceil(8).min(32);

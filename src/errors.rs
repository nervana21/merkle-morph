...
pub enum ChannelError {
    ...
    /// Challenge transaction required (old state detected)
    #[error("Challenge transaction required: old state detected with nonce {old_nonce}, latest is {latest_nonce}")]
    ChallengeTransactionRequired {
...
pub enum GlobalError {
    ...
    /// Duplicate transition witness for (wallet_id, prev_root) already exists
    #[error("Duplicate transition witness: transition witness for wallet_id {wallet_id:?} and prev_root {prev_root:?} already exists")]
    DuplicateTransitionWitnessViolation {
        /// The wallet ID
        wallet_id: WalletId,
        /// The previous global root
...
pub enum BtxError {
    ...
    /// This can happen for non-standard scripts.

...
use crate::types::{
    Bytes32, ChannelCommitment, ChannelId, CHAIN_DOMAIN, WALLET_HASH_DOMAIN, WALLET_INIT_DOMAIN,
};
...
/// Maximum number of channels supported in a single wallet transition proof
...
pub(super) fn build_transition_trace(
    ...
    let old_wallet_commitment = compute_commitment(old_wallet).map_err(|e| {
        crate::errors::Error::Zkp(crate::errors::ZkpError::TraceGenerationFailed(format!(
    ...
    let new_wallet_commitment = compute_commitment(new_wallet).map_err(|e| {
        crate::errors::Error::Zkp(crate::errors::ZkpError::TraceGenerationFailed(format!(
...
///
/// # Arguments
/// * `wallets` - Sequence of wallet states (must have length = inputs.len() + 1)
/// * `inputs` - Sequence of wallet inputs (one per transition)
pub(super) fn build_sequence_trace(
    ...
    if wallets.len() != inputs.len() + 1 {
        return Err(crate::errors::Error::Zkp(crate::errors::ZkpError::TraceGenerationFailed(
            format!(
                "Invalid sequence: {} wallets but {} inputs (expected {} inputs)",
                wallets.len(),
                inputs.len(),
                wallets.len() - 1
            ),
...
mod tests {
    ...
    fn test_build_transition_trace() {
        ...
        let new_w = apply_input(old_w_copy, &input).expect("should apply input");
        ...
        let old_commitment = compute_commitment(&old_w).expect("should compute commitment");
    ...
    fn test_build_sequence_trace() {
        ...
        let mid_w = apply_input(old_w_copy, &input1).expect("should apply input");
        ...
        let new_w = apply_input(mid_w_copy, &input2).expect("should apply input");
        ...
        let trace =
            build_sequence_trace(&wallets, &inputs).expect("failed to build sequence trace");
        let expected_cols = column_offsets::total_cols();
        ...
        let inputs_invalid = vec![input1.clone(), input2.clone()];
        let result = build_sequence_trace(&wallets_invalid, &inputs_invalid);
        assert!(result.is_err(), "Should fail when wallets.len() != inputs.len() + 1");

        // Test: Continuity between transitions
        let trace_seq =
            build_sequence_trace(&wallets, &inputs).expect("should build sequence trace");
        ...
        let input3 = WalletInput::InsertChannel {
        ...
        let mid_w3 = apply_input(old_w3_copy, &input3).expect("should apply input");
        ...
        let final_w3 = apply_input(mid_w3_copy, &input1).expect("should apply input");
        ...
        let inputs3 = vec![input3.clone(), input1.clone()];

        let trace3 =
            build_sequence_trace(&wallets3, &inputs3).expect("should build 3-transition trace");
        let trace3_1 =

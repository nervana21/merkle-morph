...
use crate::types::{
    ChannelCommitment, ChannelId, HashChainValue, CHAIN_DOMAIN, MAX_CHANNELS, WALLET_HASH_DOMAIN,
    WALLET_INIT_DOMAIN,
};
...
use crate::zkp::types::{bytes32_to_fields, Trace, Val, BYTES_PER_U32};
...
use crate::{poseidon2_hash_fixed, Bytes32, Result};
...
pub(super) fn build_transition_trace(
    ...
    let wallet_id_hash: HashChainValue =
        poseidon2_hash_fixed(&[WALLET_INIT_DOMAIN, &old_wallet.id[..]]);

    let mut old_accumulator_rebuilt: HashChainValue = wallet_id_hash;
    for (channel_id, (is_in_old, _, old_commitment, _)) in all_channels.iter() {
        if *is_in_old {
            ...
            let channel_hash_bytes: ChannelCommitment =
    ...
    let mut new_accumulator_rebuilt: HashChainValue = wallet_id_hash;
        ...
        all_channels.iter().take(num_active_channels)
    {
        ...
        let old_channel_hash_bytes: ChannelCommitment = [0u8; 32];
        ...
        let new_channel_hash_bytes: ChannelCommitment = if *is_in_new {
            poseidon2_hash_fixed(&[WALLET_HASH_DOMAIN, &channel_id[..], &new_commitment[..]])
        } else {
            [0u8; 32]
        };
        ...
        let next_new_acc = if *is_in_new {
            ...
            for (i, field) in computed_new_output.iter().enumerate() {
                ...
                next_new_acc_bytes[i * BYTES_PER_U32..(i + 1) * BYTES_PER_U32]

        ...
        p3_air::Air<crate::zkp::builder_wrapper::ColumnSliceBuilder<'a, AB>>,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let is_in_old: AB::Expr = local[IS_IN_OLD_COL].clone().into();
        let is_in_new: AB::Expr = local[IS_IN_NEW_COL].clone().into();
        ...
        let prev_old_acc: Vec<AB::Expr> =
            (PREV_OLD_ACC_START..PREV_OLD_ACC_END).map(|i| local[i].clone().into()).collect();
        let prev_new_acc: Vec<AB::Expr> =
            (PREV_NEW_ACC_START..PREV_NEW_ACC_END).map(|i| local[i].clone().into()).collect();
        let channel_id: Vec<AB::Expr> =
            (CHANNEL_ID_START..CHANNEL_ID_END).map(|i| local[i].clone().into()).collect();
        let channel_commitment: Vec<AB::Expr> = (CHANNEL_COMMITMENT_START..CHANNEL_COMMITMENT_END)
            .map(|i| local[i].clone().into())
            .collect();
        let next_old_acc: Vec<AB::Expr> =
            (NEXT_OLD_ACC_START..NEXT_OLD_ACC_END).map(|i| local[i].clone().into()).collect();
        let next_new_acc: Vec<AB::Expr> =
            (NEXT_NEW_ACC_START..NEXT_NEW_ACC_END).map(|i| local[i].clone().into()).collect();
        ...
        let computed_new_accumulator: Vec<AB::Expr> = (0..POSEIDON2_OUTPUT_SIZE)
            .map(|j| {
                ...
                local_for_poseidon[col_idx].clone().into()
            })
            .collect();
        ...
        if main.height() > 1 {
            ...
            let _next_prev_old_acc: Vec<AB::Expr> = (PREV_OLD_ACC_START..PREV_OLD_ACC_END)
                .map(|i| next_slice[i].clone().into())
                .collect();
            let next_prev_new_acc: Vec<AB::Expr> = (PREV_NEW_ACC_START..PREV_NEW_ACC_END)
                .map(|i| next_slice[i].clone().into())
                .collect();

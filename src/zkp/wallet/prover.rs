...
pub fn prove_wallet_commitment(config: &StarkConfig, wallet: &WalletState) -> Result<Proof> {
    ...
    let wallet_commitment = crate::wallet::commitment::compute_commitment(wallet)?;
...
/// * `inputs` - Sequence of inputs: `inputs[i]` transitions `wallets[i]` to `wallets[i+1]`
///
/// # Returns
...
pub fn prove_wallet_transition(
    ...
    if wallets.is_empty() {
        return Err(crate::errors::Error::Zkp(crate::errors::ZkpError::ProofGenerationFailed));
    }

    if wallets.len() != inputs.len() + 1 {
        return Err(crate::errors::Error::Zkp(crate::errors::ZkpError::ProofGenerationFailed));
    }

    // Validate sequence: apply each input and verify resulting wallet matches next
    for i in 0..inputs.len() {
        ...
        let expected_next_wallet = apply_input(wallet_copy, inputs[i])?;
    ...
    let wallets_owned: Vec<WalletState> = wallets
        ...
        .collect();
    let inputs_owned: Vec<WalletInput> = inputs.iter().map(|i| (*i).clone()).collect();

    let trace = build_sequence_trace(&wallets_owned, &inputs_owned)?;

    let initial_wallet_commitment = compute_commitment(wallets[0])?;
    let final_wallet_commitment = compute_commitment(wallets[wallets.len() - 1])?;
    ...
    match proof_result {
        Ok(proof) => Ok(proof),
        Err(_panic_payload) =>
            Err(crate::errors::Error::Zkp(crate::errors::ZkpError::ProofGenerationFailed)),
    }
}
...
mod tests {
    ...
    use crate::wallet::input::WalletInput;
    use crate::wallet::state::WalletState;
    use crate::wallet::transition::apply_input;
    use crate::zkp::types::create_config;
    use crate::zkp::wallet::public_inputs::{WalletPublicInputs, WalletTransitionPublicInputs};
    use crate::zkp::wallet::{verify_wallet_commitment, verify_wallet_transition, MAX_CHANNELS};
    ...
    fn create_test_wallet(wallet_id: u8, channels: &[(u8, u8)]) -> WalletState {
        ...
        wallet.commitment = compute_commitment(&wallet).expect("should compute commitment");
        wallet
    }
    ...
    fn test_prove_wallet_commitment() {
        ...
        // Test 1: Empty wallet
        {
            ...
            let wallet_commitment = compute_commitment(&wallet).expect("should compute commitment");
        ...
        // Verification fails with wrong wallet_commitment
        {
            ...
            let mut wrong_commitment =
                compute_commitment(&wallet).expect("should compute commitment");
    ...
    fn test_prove_wallet_transition() {
        ...
        // Empty wallet → insert channel
        {
            ...
            let input = WalletInput::InsertChannel { channel_id, channel_commitment };
            ...
            let new_wallet = apply_input(old_wallet_copy, &input).expect("Should apply input");

            let proof = prove_wallet_transition(&config, &[&old_wallet, &new_wallet], &[&input])
                .expect("Should generate proof for empty wallet → insert");

            let initial_commitment =
                compute_commitment(&old_wallet).expect("should compute commitment");
        ...
        // Update existing channel (insert with same channel_id)
        {
            ...
            let input = WalletInput::InsertChannel {
                channel_id,
...

        {
            ...
            let input1 = WalletInput::InsertChannel {
            ...
            let wallet1 = apply_input(wallet0_copy, &input1).expect("Should apply input1");

            let mut channel_id2 = [0u8; 32];
            channel_id2[31] = 3;
            let mut channel_commitment2 = [0u8; 32];
            channel_commitment2[31] = 30;
            let input2 = WalletInput::InsertChannel {
            ...
            let wallet2 = apply_input(wallet1_copy, &input2).expect("Should apply input2");
            ...
            match proof_result {
                Ok(proof) => {
                    let initial_commitment =
                        compute_commitment(&wallet0).expect("should compute commitment");
                    let final_commitment =
                        compute_commitment(&wallet2).expect("should compute commitment");

...
/// Number of bytes per u32 (used when converting bytes to BabyBear field elements)
/// BabyBear is a 31-bit field represented as a u32, so we chunk bytes into groups of 4
pub(super) const BYTES_PER_U32: usize = 4;
...
/// Type alias for public key bytes (when converting from XOnlyPublicKey)
pub type PublicKeyBytes = Bytes32;
...
pub(super) fn bytes32_to_fields(bytes: Bytes32) -> [Val; 8] {
    ...
    for (i, chunk) in bytes.chunks(BYTES_PER_U32).enumerate() {

...
pub(super) fn build_global_trace(subtrees: &[SubtreeRoot]) -> Result<Trace> {
    ...
    // Check if the last composition step is the root (depth 0)
    // If not, we need to ensure the last row has the global root by adding padding
    let last_step_is_root = composition_steps.last().map(|step| step.depth == 0).unwrap_or(false);

    // If the last step is not the root, we need at least one padding row with the global root
    // So we pad to at least current_rows + 1, then round up to next power of 2
    let min_rows_needed = if !last_step_is_root && current_rows > 0 {
        (current_rows + 1).max(MIN_ROWS)
    } else {
        current_rows.max(MIN_ROWS)
    };
    let num_rows = min_rows_needed.next_power_of_two();

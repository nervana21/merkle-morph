...
impl WalletsWithProofs {
    ...
    pub fn try_insert(
        ...
        let wallet_commitment = compute_commitment(&state)?;
...
pub fn verify_global_root(
    ...
    for (wallet_id, (state, proof)) in wallets_with_proofs.iter() {
        let wallet_commitment: WalletCommitment = compute_commitment(state)?;
...
mod tests {
    ...
    fn test_verify_global_root() {
        ...
        // Success case: Wallets with proofs + non-overlapping subtree_roots
        {
            ...
            let subtree =
                compute_subtree_root(&other_wallet_commitments, other_wallet.id, other_wallet.id)
        ...
        // Error case: Overlapping subtrees
        {
            ...
            wallet_id_max[31] = 15;
            let subtree1 = compute_subtree_root(&subtree1_wallets, wallet_id_min, wallet_id_max)
                .expect("Should compute subtree root");
            ...
            let subtree2 = compute_subtree_root(&subtree2_wallets, wallet_id_min2, wallet_id_max2)
                .expect("Should compute subtree root");

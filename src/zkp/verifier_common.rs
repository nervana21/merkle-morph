...
use crate::zkp::types::{bytes32_to_fields, PublicKeyBytes, Val};
use crate::Bytes32;

/// Build public values vector from an ID and a commitment
///
/// This function converts an ID and a commitment to field elements
...
pub(crate) fn build_public_values_from_id_commitment_and_pubkey(
    ...
    // Convert XOnlyPublicKey (32 bytes) to PublicKeyBytes, then to fields
    let pubkey_bytes: PublicKeyBytes = sender_pubkey.serialize();

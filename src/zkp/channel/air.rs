...
///
/// The trace structure follows the dependency chain:
/// - Columns 0-4: Channel state (nonce, is_closed, sender_balance, receiver_balance, amount)
/// - Columns STATE_HASH_OFFSET+: Poseidon2 AIR columns for state_hash = poseidon2(sender_balance || receiver_balance || sender_pubkey || receiver_pubkey || metadata || is_closed)
/// - Columns commitment_start()..commitment_end(): commitment[8] (computed from state_hash + nonce)
/// - Columns commitment_offset()+: Poseidon2 AIR columns for commitment (two-stage hash):
///   stage1 = poseidon2(CHANNEL_DOMAIN_TAG || channel_id || state_hash),
///   commitment = poseidon2(stage1 || nonce)
/// - Columns auth_hash_start()..auth_hash_end(): auth_hash[8] (computed from channel_id + nonce + commitment + sender_sk)
///
        ...
        p3_air::Air<crate::zkp::builder_wrapper::ColumnSliceBuilder<'a, AB>>,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let sender_balance_old = local[column_offsets::SENDER_BALANCE].clone();
        let receiver_balance_old = local[column_offsets::RECEIVER_BALANCE].clone();
        let nonce_old = local[column_offsets::NONCE].clone();
        let amount_old = local[column_offsets::AMOUNT].clone();
        let is_closed_old = local[column_offsets::IS_CLOSED].clone();
        let commitment_start = column_offsets::commitment_start();
        let commitment_end = column_offsets::commitment_end();
        let commitment_old: Vec<AB::Expr> =
            (commitment_start..commitment_end).map(|i| local[i].clone().into()).collect();

        let sender_balance_new = next[column_offsets::SENDER_BALANCE].clone();
        let receiver_balance_new = next[column_offsets::RECEIVER_BALANCE].clone();
        let nonce_new = next[column_offsets::NONCE].clone();
        let amount_new = next[column_offsets::AMOUNT].clone();
        let is_closed_new = next[column_offsets::IS_CLOSED].clone();
        let commitment_new: Vec<AB::Expr> =
            (commitment_start..commitment_end).map(|i| next[i].clone().into()).collect();
        let auth_hash_start = column_offsets::auth_hash_start();
        let auth_hash_end = column_offsets::auth_hash_end();
        let auth_hash_new: Vec<AB::Expr> =
            (auth_hash_start..auth_hash_end).map(|i| next[i].clone().into()).collect();

        let amount_field = amount_new.clone();
        ...
        let nonce_diff = nonce_new.clone() - nonce_old.clone();
        ...
        // Detect closing transitions: when is_closed changes from 0 to 1
        // For closing operations, both parties can contribute to fees
        let is_closing =
            (is_closed_new.clone() - is_closed_old.clone()) * is_real_transition.clone();
        let is_transfer = is_real_transition.clone() - is_closing.clone();

        let delta_sender = AB::Expr::ZERO - amount_field.clone();
        let delta_receiver = amount_field.clone();

        // For transfers: sender loses amount, receiver gains amount
        // For closing: sender loses sender_contribution, receiver loses receiver_contribution
        // The amount field represents total_fee, but we need to compute contributions from balance changes
        let sender_contribution = sender_balance_old.clone() - sender_balance_new.clone();
        let receiver_contribution = receiver_balance_old.clone() - receiver_balance_new.clone();
        let total_contribution = sender_contribution.clone() + receiver_contribution.clone();

        // For closing: sender loses sender_contribution (computed from balance change)
        // For transfer: sender loses amount (standard transfer)
        let real_sender_transfer =
            sender_balance_new.clone() - sender_balance_old.clone() - delta_sender.clone();
        let real_sender_closing =
            sender_balance_new.clone() - sender_balance_old.clone() + sender_contribution.clone();
        let real_receiver_transfer =
            receiver_balance_new.clone() - receiver_balance_old.clone() - delta_receiver.clone();
        // For closing: receiver loses receiver_contribution (computed from balance change)
        let real_receiver_closing = receiver_balance_new.clone() - receiver_balance_old.clone()
            + receiver_contribution.clone();
        // For closing: total contributions must equal total_fee (amount)
        let real_closing_fee_match = total_contribution - amount_field.clone();
        let real_nonce = nonce_new.clone() - nonce_old.clone() - AB::Expr::ONE;

        let padding_sender = sender_balance_new.clone() - sender_balance_old.clone();
        let padding_receiver = receiver_balance_new.clone() - receiver_balance_old.clone();
        let padding_nonce = nonce_new.clone() - nonce_old.clone();
        let padding_amount = amount_new.clone() - amount_old.clone();
        ...
        // - For closing operations: sender loses sender_contribution, receiver loses receiver_contribution
        // - For transfer operations: sender loses amount, receiver gains amount
        let sender_constraint = is_transfer.clone() * real_sender_transfer.clone()
            + is_closing.clone() * real_sender_closing.clone()
            + is_padding.clone() * padding_sender.clone();
        let receiver_constraint = is_transfer.clone() * real_receiver_transfer.clone()
            + is_closing.clone() * real_receiver_closing.clone()
            + is_padding.clone() * padding_receiver.clone();
        // For closing operations, verify that total contributions equal total_fee
        let closing_fee_constraint = is_closing.clone() * real_closing_fee_match.clone();
        ...
        when_transition.assert_zero(closing_fee_constraint);
        ...
        let auth_hash_old: Vec<AB::Expr> =
            (auth_hash_start..auth_hash_end).map(|i| local[i].clone().into()).collect();

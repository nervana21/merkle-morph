...
mod tests {
    ...
    fn test_verify_channel_transition() {
        let config = create_config().expect("STARK config should always succeed");
        ...
        let amount = TransferAmount::new(30).expect("transfer amount should always succeed");
        let new_state = apply_transfer_state_only(&old_state, &amount)
            .expect("transfer state should always succeed");
        let proof = prove_channel_transition(
            channel_id, &old_state, &amount, &new_state, &sender_sk, &config,
        )
        .expect("channel transition proof should always succeed");

...
pub fn insert_wallet(
    ...
    global_state.nonce = global_state
        .nonce
        .checked_add(1)
        .ok_or(GlobalError::GlobalNonceOverflow)?;
    Ok(())
}
...
pub fn get_wallet<'a>(
    ...
    global_state
        .wallets
        .get(wallet_id)
        .ok_or(GlobalError::WalletNotFound(*wallet_id))
}
...
mod tests {
    ...
    fn test_insert_wallet() {
        ...
        assert!(matches!(
            nonce_overflow,
            Err(GlobalError::GlobalNonceOverflow)
        ));
    }
    ...
    fn test_get_wallet() {
        ...
        assert_eq!(
            non_existent.unwrap_err(),
            GlobalError::WalletNotFound(non_existent_id)
        );
    }
}

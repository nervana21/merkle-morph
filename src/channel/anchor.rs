...
/// to be economical to sweep when fee rates decrease). This implementation
/// uses 330 sats, which aligns with typical implementations. The value
/// can be customized via policy modules that take fee rates and dust
/// limits into account when using `build_anchor_output()` with a custom value.
...
pub fn build_anchor_output_default() -> TxOut {
    TxOut {
        amount: Amount::from_sat(DEFAULT_ANCHOR_VALUE_SATS)
            .expect("DEFAULT_ANCHOR_VALUE_SATS out of range"),
...
pub fn build_anchor_output(value_sats: u64) -> TxOut {
    TxOut {
        amount: Amount::from_sat(value_sats).expect("value_sats out of range"),
        script_pubkey: build_anchor_script(),
    }
}
...
mod tests {
    ...
    fn test_build_anchor_output_default() {
        ...
        assert_eq!(output.amount.to_sat(), DEFAULT_ANCHOR_VALUE_SATS);
    }
    ...
    fn test_build_anchor_output() {
        ...
        assert_eq!(output.amount.to_sat(), value_sats);
    }
}

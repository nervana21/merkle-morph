...
impl Open {
    ...
    pub fn with_timeout(
        ...
        let sender_keypair = sender_revocation_secret.keypair();
        let (sender_revocation_pubkey, _) = XOnlyPublicKey::from_keypair(&sender_keypair);
        let receiver_keypair = receiver_revocation_secret.keypair();
        let (receiver_revocation_pubkey, _) = XOnlyPublicKey::from_keypair(&receiver_keypair);

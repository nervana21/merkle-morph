...
///
/// // Generate proof
/// let result = apply_transfer(&state, &amount, channel_id, &config, &sender_sk)?;
...
mod tests {
    ...
    fn test_apply_transfer_state_only() {
        ...
        let metadata_max_state = Open {
            ...
            nonce: 0,
            commitment: ChannelCommitment::default(),
            metadata: vec![0u8; crate::types::MAX_METADATA_SIZE],
            timeout_blocks: 144,
        };

        assert!(apply_transfer_state_only(&metadata_max_state, &amount).is_ok());
    ...
    fn test_apply_transfer() {
        ...
        assert!(matches!(
            apply_transfer(&metadata_too_large_state, &amount, channel_id, &config, &sender_sk),
            Err(crate::Error::Channel(crate::errors::ChannelError::MetadataTooLarge { .. }))
        ));
        ...
        assert!(
            apply_transfer(&metadata_max_state, &amount, channel_id, &config, &sender_sk).is_ok()
        );
        ...
        assert!(matches!(
            apply_transfer(&insufficient_state, &amount, channel_id, &config, &sender_sk),
            Err(crate::Error::Channel(crate::errors::ChannelError::InsufficientBalance))
        ));
        ...
        assert!(matches!(
            apply_transfer(&balance_overflow_state, &amount, channel_id, &config, &sender_sk),
            Err(crate::Error::Channel(crate::errors::ChannelError::BalanceOverflow))
        ));
        ...
        assert!(matches!(
            apply_transfer(&nonce_overflow_state, &amount, channel_id, &config, &sender_sk),
            Err(crate::Error::Channel(crate::errors::ChannelError::ChannelNonceOverflow))
        ));
        ...
        let result =
            apply_transfer(&valid_state, &amount, channel_id, &config, &sender_sk).expect("valid");

...
use crate::zkp::prove_recover_transition;
...
impl fmt::Debug for RecoverResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RecoverResult")
...
///
/// # Returns
/// * `Ok(RecoverResult)` - Contains new state, commitment, and proof
...
pub fn apply_recover(
    ...
    let commitment = closed_state.commitment;

    let proof = prove_recover_transition(channel_id, state, &closed_state, config)?;

    Ok(RecoverResult { new_state: closed_state, commitment, proof })
}
...
mod tests {
    ...
    fn test_apply_recover() {
        ...
        let result = apply_recover(&state, channel_id, &config).expect("recover should succeed");

        assert_eq!(result.new_state.sender_balance, 0);
        assert_eq!(result.new_state.receiver_balance, 100);
        assert_eq!(result.new_state.total_capacity, 100);

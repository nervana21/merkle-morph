...
use bitcoin::script::{ScriptBuf, ScriptPubKeyBuf, ScriptSigBuf};
...
pub fn build_all_challenge_transactions(
    ...
    input_script_pubkeys: Vec<ScriptPubKeyBuf>,
) -> Result<Vec<Transaction>> {
    if old_force_close_tx.outputs.is_empty() {
    ...
    if input_private_keys.len() != old_force_close_tx.outputs.len() {
        return Err(ChannelError::InvalidChallengeParameters {
            reason: format!(
                ...
                old_force_close_tx.outputs.len()
            ),
        }
        .into());
    }

    if input_script_pubkeys.len() != old_force_close_tx.outputs.len() {
        return Err(ChannelError::InvalidChallengeParameters {
            reason: format!(
    ...
    let challenge_txs: Result<Vec<_>> = (0..old_force_close_tx.outputs.len())
        .map(|output_index| {
...
pub fn build_challenge_transaction_for_output(
    ...
    input_script_pubkey: ScriptPubKeyBuf,
) -> Result<Transaction> {
    if output_index >= old_force_close_tx.outputs.len() {
        return Err(ChannelError::InvalidForceCloseTransaction {
            reason: format!(
    ...
    let output = &old_force_close_tx.outputs[output_index];
    let output_value = output.amount.to_sat();
    ...
    let challenge_output = TxOut {
        amount: Amount::from_sat(challenge_value).expect("challenge value out of range"),
        script_pubkey: sp_script_pubkey,
    };
...
pub fn build_revocation_sweep_transaction(
    ...
    let input = TxIn {
        ...
        sequence: Sequence::MAX,
        witness: Witness::new(), // Witness will be populated with challenge signature + control block
    };

    let output = TxOut {
        amount: Amount::from_sat(sweep_value).expect("sweep value out of range"),
        script_pubkey: sp_script_pubkey,
    };
...
mod tests {
    ...
    fn test_build_all_challenge_transactions() {
        ...
        let (input_sk2, input_spk2) = test_taproot_input_keypair();
        ...
        let one_output = Transaction {
            ...
            inputs: vec![],
            outputs: vec![TxOut {
                amount: Amount::from_sat(3).expect("valid amount"),
        ...
        let two_output = Transaction {
            ...
            outputs: vec![
                ...
                TxOut {
                    amount: Amount::from_sat(75_000).expect("valid amount"),
                    script_pubkey: ScriptPubKeyBuf::new(),
                },
            ],
        };
        ...
        assert_eq!(success[0].version, Version::THREE);
        assert_eq!(success[1].version, Version::THREE);
        assert_eq!(success[0].outputs[0].amount, Amount::from_sat(49_000).expect("valid amount"));
        assert_eq!(success[1].outputs[0].amount, Amount::from_sat(74_000).expect("valid amount"));
    }
    ...
    fn test_build_challenge_transaction_for_output() {
        ...
        let old_tx = Transaction {
            ...
            inputs: vec![],
            outputs: vec![TxOut {
                amount: Amount::from_sat(100_000).expect("valid amount"),
                script_pubkey: ScriptPubKeyBuf::new(),
            }],
        };
        ...
        let dust = build_challenge_transaction_for_output(
            ...
            200,
            sp_code.clone(),
            SecretKey::from_secret_bytes([7u8; 32]).expect("valid secret key"),
            input_spk,
        );
        ...
        let input_sk =
            bitcoin::secp256k1::SecretKey::from_secret_bytes([8u8; 32]).expect("valid secret key");
        let input_keypair = input_sk.keypair();
        let (input_x_only_pk, _) = bitcoin::key::XOnlyPublicKey::from_keypair(&input_keypair);
        ...
        assert_eq!(success.version, Version::THREE);
        assert_eq!(success.outputs[0].amount, Amount::from_sat(99_000).expect("valid amount"));
        assert_eq!(success.inputs.len(), 1);
    }
    ...
    fn test_build_revocation_sweep_transaction() {
        ...
        .expect("funding");
        funding.update_funding_utxo(bitcoin::Txid::from_byte_array([1u8; 32]), 0, 100_000);
        ...
        assert_eq!(sweep.version, Version::THREE);
        assert_eq!(sweep.outputs[0].amount, Amount::from_sat(99_000).expect("valid amount"));

...
use crate::types::{Bytes32, ChannelId};
...
impl ChannelState {
    ...
    fn transfer(&self, amount: u64) -> Result<Self, ChannelError> {
        ...
        let next_sender_balance =
            self.sender_balance.checked_sub(amount).ok_or(ChannelError::InsufficientBalance)?;
        let next_receiver_balance =
            self.receiver_balance.checked_add(amount).ok_or(ChannelError::BalanceOverflow)?;
    ...
    pub fn apply_transfer(&self, channel_id: ChannelId, amount: u64) -> Result<Self, ChannelError> {
        ...
        next_state.nonce = self.nonce.checked_add(1).ok_or(ChannelError::ChannelNonceOverflow)?;
...
mod tests {
    ...
    fn test_transfer() {
        ...
        assert!(matches!(zero_transfer, Err(ChannelError::InvalidZeroTransfer)));
        ...
        assert_eq!(next_state_with_metadata.nonce, initial_state_with_metadata.nonce);

        // Test error cases
        // Insufficient balance
        let small_initial_state = ChannelState::new(10);
        let transfer_result = small_initial_state.transfer(21);
        assert!(matches!(transfer_result, Err(ChannelError::InsufficientBalance)));
        ...
        assert!(matches!(transfer_result, Err(ChannelError::BalanceOverflow)));
        ...
        assert!(matches!(transfer_result, Err(ChannelError::InvalidZeroTransfer)));
    ...
    fn test_apply_transfer() {
        ...
        assert!(matches!(overflow_result, Err(ChannelError::ChannelNonceOverflow)));
    }
}
